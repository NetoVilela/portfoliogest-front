...
export function addProduct(product: ProductCardProps, products: ProductCardProps[]) {
  return async () => {
    try {
      const response = await api.post('/api/cart/add', { product, products });
...
export function removeProduct(id: string | number | undefined, products: ProductCardProps[]) {
  return async () => {
    try {
      const response = await api.post('/api/cart/remove', { id, products });
...
export function updateProduct(id: string | number | undefined, quantity: number, products: ProductCardProps[]) {
  return async () => {
    try {
      const response = await api.post('/api/cart/update', { id, quantity, products });
...
export function setBillingAddress(address: Address | null) {
  return async () => {
    try {
      const response = await api.post('/api/cart/billing-address', { address });
...
export function setDiscount(code: string, total: number) {
  return async () => {
    try {
      const response = await api.post('/api/cart/discount', { code, total });
...
export function setShippingCharge(charge: string, shipping: number) {
  return async () => {
    try {
      const response = await api.post('/api/cart/shipping-charge', { charge, shipping });
...
export function setPaymentMethod(method: string) {
  return async () => {
    try {
      const response = await api.post('/api/cart/payment-method', { method });
...
export function setPaymentCard(card: string) {
  return async () => {
    try {
      const response = await api.post('/api/cart/payment-card', { card });
...
export function resetCart() {
  return async () => {
    try {
      const response = await api.post('/api/cart/reset');
