...
export function getInvoiceInsert(NewLists: InvoiceList) {
  return async () => {
    try {
      const response = await api.post('/api/invoice/insert', { list: NewLists });
...
export function getInvoiceUpdate(NewLists: InvoiceList) {
  return async () => {
    try {
      const response = await api.post('/api/invoice/update', { list: NewLists });
...
export function getInvoiceDelete(invoiceId: number) {
  return async () => {
    try {
      await api.post('/api/invoice/delete', { invoiceId });
...
export function getInvoiceSingleList(invoiceId: any) {
  return async () => {
    try {
      const response = await api.post('/api/invoice/single', { id: invoiceId });
